...
export function OverviewWorkspace({ initialAccountId }: { initialAccountId?: string }) {
  ...
  return <div className="space-y-8">
    <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div className="max-w-3xl"><p className="mb-3 text-sm font-medium text-accent">Live account overview</p><h1 className="text-3xl font-semibold tracking-tight lg:text-4xl">A clear view of what needs attention.</h1><p className="mt-3 max-w-2xl text-base leading-7 text-muted">Balances and activity below come from the authenticated backend account and transaction endpoints. Amounts remain separated by currency.</p></div><div className="w-full max-w-md"><AccountSelect accounts={accountsQuery.data} value={selectedAccount.id} onChange={selectAccount} label="Account context" id="overview-account" isFetching={accountsQuery.isFetching} error={accountsQuery.error} onRetry={() => void accountsQuery.refetch()} /></div></div>
    ...
    <section aria-labelledby="account-summary-title"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-medium text-accent">Authoritative account data</p><h2 id="account-summary-title" className="mt-1 text-xl font-semibold">Your accounts</h2></div><Link href={`/transactions?accountId=${encodeURIComponent(selectedAccount.id)}` as Route} className="text-sm font-semibold text-accent hover:underline">Open transactions</Link></div><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{accountsQuery.data.map(account => <AccountCard key={account.id} account={account} selected={account.id === selectedAccount.id} onSelect={() => selectAccount(account.id)} />)}</div></section>
    ...
    <section aria-labelledby="attention-title" className="rounded-2xl border border-border bg-surface p-6 lg:p-8"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-medium text-accent">Selected account activity</p><h2 id="attention-title" className="mt-1 text-xl font-semibold">Recent transactions</h2></div><Link href={`/transfers?accountId=${encodeURIComponent(selectedAccount.id)}` as Route} className="text-sm font-semibold text-accent hover:underline">Start a transfer</Link></div>{activityQuery.isPending && <div role="status" className="mt-5 rounded-xl bg-surface-muted p-5 text-sm text-muted">Loading recent backend-confirmed activity…</div>}{activityQuery.isError && !activityQuery.data && <div role="alert" className="mt-5 rounded-xl border border-red-200 bg-red-50 p-5 text-sm text-[var(--aries-danger)]"><div className="flex items-start gap-3"><AlertTriangle aria-hidden="true" size={18} /><div><p className="font-medium">Recent activity could not be loaded</p><p className="mt-1 text-muted">No activity state is inferred. Retry when the service is available.</p><Button variant="secondary" className="mt-4" onClick={() => void activityQuery.refetch()}>Try again</Button></div></div></div>}{activityQuery.isError && activityQuery.data && <p role="alert" className="mt-5 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-[var(--aries-warning)]">Activity refresh failed; showing the last confirmed records.</p>}{activityQuery.isFetching && !activityQuery.isPending && <p role="status" className="mt-5 flex items-center gap-2 text-sm text-muted"><RefreshCw aria-hidden="true" size={15} className="animate-spin" />Refreshing activity; current records remain visible.</p>}{activityQuery.data && <ActivityList accountId={selectedAccount.id} transactions={activityQuery.data.content} />}</section>
  </div>;
}
...
function ActivityList({ accountId, transactions }: { accountId: string; transactions: Array<Pick<Transaction, "id" | "amount" | "currency" | "status" | "createdAt" | "failureReason">> }) {
  ...
  return <div className="mt-5 divide-y divide-border rounded-xl border border-border">{transactions.map(transaction => <Link key={transaction.id} href={`/transactions?accountId=${encodeURIComponent(accountId)}&transactionId=${encodeURIComponent(transaction.id)}` as Route} className="flex flex-col gap-2 px-4 py-4 hover:bg-surface-muted sm:flex-row sm:items-center sm:justify-between"><div><p className="font-mono text-xs">{transaction.id}</p><time dateTime={transaction.createdAt} className="text-xs text-muted">{formatDate(transaction.createdAt)}</time></div><div className="flex items-center justify-between gap-4 sm:justify-end"><span className="font-mono text-sm tabular-nums">{formatMoney(transaction.amount, transaction.currency)}</span><TransactionStatusBadge transaction={{ status: transaction.status, failureReason: transaction.failureReason }} /></div></Link>)}</div>;
}
...
function EmptyAccounts({ detail = "No account was returned for the authenticated user. No balance is inferred." }: { detail?: string }) { return <section role="status" className="rounded-2xl border border-dashed border-border bg-surface p-10 text-center"><p className="font-medium">No accounts available</p><p className="mt-2 text-sm leading-6 text-muted">{detail}</p></section>; }
function LoadError({ error, onRetry }: { error: Error; onRetry: () => void }) { const denied = error instanceof ApiError && (error.kind === "forbidden" || error.kind === "unauthorized"); return <section role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-6"><p className="font-medium text-[var(--aries-danger)]">{denied ? "Account access is unavailable" : "Accounts could not be loaded"}</p><p className="mt-2 text-sm text-muted">{denied ? "Sign in again or contact an authorized operator. No account state was changed." : "No balance is inferred. Retry when the service is available."}</p><Button variant="secondary" className="mt-4" onClick={onRetry}>Try again</Button></section>; }
