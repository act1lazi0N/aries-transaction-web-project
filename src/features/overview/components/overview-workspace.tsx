...
export function OverviewWorkspace({ initialAccountId }: { initialAccountId?: string }) {
  ...
  return <div className="space-y-8">
    ...
    <section aria-labelledby="account-summary-title"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-medium text-accent">Your account information</p><h2 id="account-summary-title" className="mt-1 text-xl font-semibold">Your accounts</h2></div><Link href={`/transactions?accountId=${encodeURIComponent(selectedAccount.id)}` as Route} className="text-sm font-semibold text-accent hover:underline">View transactions</Link></div><div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{accountsQuery.data.map(account => <AccountCard key={account.id} account={account} selected={account.id === selectedAccount.id} onSelect={() => selectAccount(account.id)} />)}</div></section>
