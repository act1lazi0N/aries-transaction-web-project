    ...
    <section aria-labelledby="reconciliation-request-title" className="rounded-2xl border border-border bg-surface p-6">
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-muted">Compare completed transaction records for a defined time window. The backend owns the result and any exceptions.</p>
        </div>
      </div>
      <form onSubmit={submitDraft} className="mt-6 space-y-5" aria-describedby="reconciliation-help">
        <p id="reconciliation-help" className="text-xs leading-5 text-muted">Times are sent to the backend as UTC offsets. Running reconciliation does not itself change transaction state.</p>
...
function RunPanel({ run, isLoading, isFetching, error, onRetry, onClear }: { run?: ReconciliationRun; isLoading: boolean; isFetching: boolean; error: Error | null; onRetry: () => void; onClear: () => void }) {
  ...
  if (error && !run) return <section role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-6"><div className="flex gap-3"><AlertTriangle aria-hidden="true" className="mt-0.5 text-[var(--aries-danger)]" size={18} /><div><p className="font-medium text-[var(--aries-danger)]">The reconciliation run could not be loaded</p><p className="mt-1 text-sm leading-6 text-muted">{error instanceof ApiError && (error.kind === "forbidden" || error.kind === "unauthorized") ? "You are not authorized to view this run. No run state was changed." : "The current run status is unavailable. No result is inferred."}</p><div className="mt-4 flex gap-2"><Button type="button" variant="secondary" onClick={onRetry}>Try again</Button><Button type="button" variant="ghost" onClick={onClear}>Clear run</Button></div></div></div></section>;
  if (!run) return null;
...
function RunResult({ run, isFetching, onClear, onRetry }: { run: ReconciliationRun; isFetching: boolean; onClear: () => void; onRetry: () => void }) {
  ...
  return <section aria-labelledby="reconciliation-result-title" className="rounded-2xl border border-border bg-surface p-6">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div><p className="text-sm font-medium text-accent">Backend-authoritative result</p><h2 id="reconciliation-result-title" className="mt-1 text-xl font-semibold">Reconciliation run</h2></div>
      <div className="flex items-center gap-2"><Badge tone={tone}>{lifecycle.label}</Badge><Button type="button" variant="ghost" onClick={onClear}>New run</Button></div>
    </div>
    <div className="mt-5 flex flex-wrap items-center gap-3 text-xs text-muted"><span className="break-all font-mono">Run {run.id}</span>{isFetching && <span role="status" className="inline-flex items-center gap-1"><RefreshCw aria-hidden="true" size={13} className="animate-spin" />Refreshing</span>}<Button type="button" variant="ghost" className="min-h-7 px-2" onClick={onRetry}>Refresh</Button></div>
    ...
    {lifecycle.kind === "running" && <p role="status" className="mt-5 rounded-xl bg-amber-50 p-4 text-sm text-[var(--aries-warning)]">The backend is still processing this run. Completed status is not inferred.</p>}
    {lifecycle.kind === "unknown" && <p role="alert" className="mt-5 rounded-xl bg-amber-50 p-4 text-sm text-[var(--aries-warning)]">The backend returned an unrecognized status. No success or failure is inferred.</p>}
    <div className="mt-8 flex flex-wrap items-end justify-between gap-3"><div><h3 className="font-semibold">Exceptions</h3><p className="mt-1 text-sm text-muted">Only exception records returned by the backend are shown.</p></div>{exceptionTypes.length > 0 && <label className="text-sm font-medium">Filter by type<select value={filter} onChange={(event) => setFilter(event.target.value)} className="mt-2 block h-10 rounded-lg border border-border bg-surface px-3 text-sm font-normal outline-none focus:border-accent"><option value="ALL">All types</option>{exceptionTypes.map((type) => <option key={type} value={type}>{type.replaceAll("_", " ")}</option>)}</select></label>}</div>
    {filteredExceptions.length === 0 ? <div className="mt-4 rounded-xl border border-dashed border-border p-8 text-center"><p className="font-medium">{run.exceptions.length === 0 ? "No exceptions reported" : "No exceptions match this filter"}</p><p className="mt-2 text-sm text-muted">{run.exceptions.length === 0 ? "The backend returned no reconciliation exceptions for this run." : "Choose another exception type or clear the filter."}</p></div> : <div className="mt-4 overflow-x-auto rounded-xl border border-border"><Table><TableHeader><TableRow>{table.getHeaderGroups()[0]?.headers.map((header) => <TableHead key={header.id}>{flexRender(header.column.columnDef.header, header.getContext())}</TableHead>)}</TableRow></TableHeader><TableBody>{table.getRowModel().rows.map((row) => <TableRow key={row.id}>{row.getVisibleCells().map((cell) => <TableCell key={cell.id}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</TableCell>)}</TableRow>)}</TableBody></Table></div>}
  </section>;
}
