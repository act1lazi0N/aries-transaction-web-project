...
export function AccountSelect({ accounts, value, onChange, label = "Account", id = "account-select", excludeAccountId, disabled, isLoading, isFetching, error, onRetry, validationError }: Props) {
  ...
  return <div className="space-y-2">
    <label htmlFor={id} className="block text-sm font-medium">{label}
      <select id={id} value={value ?? ""} onChange={event => onChange(event.target.value)} disabled={disabled || isLoading} aria-describedby={`${descriptionId}${validationError ? ` ${validationId}` : ""}`} aria-invalid={Boolean(validationError)} className="mt-2 h-11 w-full rounded-lg border border-border bg-surface px-3 text-sm outline-none focus:border-accent">
    ...
    {validationError && <p id={validationId} role="alert" className="text-xs text-[var(--aries-danger)]">{validationError}</p>}
  </div>;
}
