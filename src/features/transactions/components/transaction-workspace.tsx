...
export function TransactionWorkspace({ accountId, transactionId, page, size, sort }: Props) {
  ...
  const canRefund = session.user?.role === "OPERATOR" || session.user?.role === "ADMIN";
  ...
  const columns = useMemo<ColumnDef<Transaction>[]>(() => [
    ...
    { id: "action", header: "Action", cell: ({ row }) => row.original.status === "COMPLETED" ? <div className="flex flex-wrap gap-2"><Button variant="secondary" className="min-h-9 px-3 text-xs" onClick={event => { reversalTriggerRef.current = event.currentTarget; reverseMutation.reset(); setReversalKey(idempotencyKey()); setSelected(row.original); }}>Review reversal</Button>{canRefund && <Button variant="danger" className="min-h-9 px-3 text-xs" onClick={event => { refundTriggerRef.current = event.currentTarget; refundMutation.reset(); setRefundKey(idempotencyKey()); setRefundTarget(row.original); }}>Review refund</Button>}</div> : <span className="text-xs text-muted">No action</span> },
  ], [canRefund, refundMutation, reverseMutation, updateUrl]);
...
function RefundDialog({ transaction, result, isPending, error, returnFocusRef, onClose, onConfirm }: { transaction: Transaction; result?: Transaction; isPending: boolean; error: Error | null; returnFocusRef: RefObject<HTMLButtonElement | null>; onClose: () => void; onConfirm: (amount: string, description?: string) => void }) {
  ...
  const lifecycle = result ? toTransactionLifecycle(result) : null;
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/30 p-6" role="presentation"><section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="refund-title" className="w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-xl"><h2 id="refund-title" className="text-lg font-semibold">Review refund</h2><p className="mt-2 text-sm leading-6 text-muted">This requests a refund. The original transaction remains unchanged until the backend confirms the refund.</p><dl className="mt-4 space-y-2 rounded-xl bg-surface-muted p-4 text-sm"><div className="flex justify-between gap-4"><dt className="text-muted">Original amount</dt><dd className="font-mono font-semibold">{formatAmount(transaction.amount, transaction.currency)}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted">Refund currency</dt><dd>{transaction.currency}</dd></div></dl><label htmlFor="refund-amount" className="mt-4 block text-sm font-medium">Refund amount</label><input id="refund-amount" inputMode="decimal" value={amount} onChange={event => setAmount(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 font-mono" aria-describedby="refund-help" disabled={isPending || Boolean(result)} /><p id="refund-help" className="mt-1 text-xs text-muted">Use the exact decimal amount; the backend enforces the remaining refundable amount.</p><label htmlFor="refund-description" className="mt-4 block text-sm font-medium">Reason (optional)</label><input id="refund-description" value={description} onChange={event => setDescription(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3" maxLength={255} disabled={isPending || Boolean(result)} />{isPending && <p role="status" className="mt-3 text-sm text-[var(--aries-warning)]">Request submitted; refund status is still processing.</p>}{result && <p role="status" className="mt-3 text-sm text-muted">Backend status: {lifecycle?.label ?? "Status unavailable"}. Completed styling is shown only from that response.</p>}{error && <p role="alert" className="mt-3 text-sm text-[var(--aries-danger)]">The refund request failed. No completed refund is shown. Retry explicitly or close this dialog.</p>}<div className="mt-6 flex justify-end gap-2"><Button ref={closeRef} variant="ghost" onClick={onClose} disabled={isPending}>{result || error ? "Close" : "Cancel"}</Button>{!result && <Button variant="danger" onClick={() => onConfirm(amount, description || undefined)} disabled={isPending || !valid}>{isPending ? "Requesting…" : "Request refund"}</Button>}</div></section></div>;
}
