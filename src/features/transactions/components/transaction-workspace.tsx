...
import { ApiError, financialMutationErrorMessage } from "@/lib/api/errors";
...
import { toTransactionLifecycle, type Transaction, type TransactionRead } from "@/features/transactions/types";
...
export function TransactionWorkspace({ accountId, transactionId, page, size, sort }: Props) {
  ...
  const columns = useMemo<ColumnDef<TransactionRead>[]>(() => [
    { id: "party", header: "Direction / party", cell: ({ row }) => <TransactionRouteSummary transaction={row.original} /> },
  ...
  if (!accountId) return <div className="space-y-5"><section className="max-w-md rounded-2xl border border-border bg-surface p-5"><AccountSelect accounts={accountsQuery.data ?? []} value={accountId} onChange={nextAccountId => updateUrl({ accountId: nextAccountId, page: 0, transactionId: undefined })} label="Account" id="transactions-account" isLoading={accountsQuery.isPending} isFetching={accountsQuery.isFetching} error={accountsQuery.error} onRetry={() => void accountsQuery.refetch()} /></section>{transactionId ? <TransactionDetail transactionId={transactionId} onClose={() => updateUrl({ transactionId: undefined })} /> : <EmptyState title="Choose an account to view transactions" detail="We only show transactions for an account available to you." />}</div>;
  ...
  return <div className="space-y-5">
    <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div className="max-w-md"><AccountSelect accounts={accountsQuery.data ?? []} value={accountId} onChange={nextAccountId => updateUrl({ accountId: nextAccountId, page: 0, transactionId: undefined })} label="Account" id="transactions-account" isLoading={accountsQuery.isPending} isFetching={accountsQuery.isFetching} error={accountsQuery.error} onRetry={() => void accountsQuery.refetch()} /><p className="mt-2 text-xs text-muted">Account number <span className="font-mono">{selectedAccount?.accountNumber ?? "Unavailable"}</span> · {query.data.totalElements} total records</p></div><div className="flex items-center gap-2"><label htmlFor="page-size" className="text-sm text-muted">Rows</label><select id="page-size" value={size} onChange={event => updateUrl({ size: Number(event.target.value), page: 0 })} className="h-10 rounded-lg border border-border bg-surface px-3 text-sm"><option value="20">20</option><option value="50">50</option><option value="100">100</option></select><Button variant="ghost" aria-label="Refresh transactions" onClick={() => void query.refetch()} disabled={query.isFetching}><RefreshCw aria-hidden="true" size={17} className={query.isFetching ? "animate-spin" : ""} /></Button></div></div>
    {query.isError && <div role="alert" className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-[var(--aries-warning)]"><span>We could not refresh your transactions, so we are showing the last available information.</span><Button variant="secondary" onClick={() => void query.refetch()}>Try again</Button></div>}
    {query.isFetching && <p role="status" className="flex items-center gap-2 text-sm text-muted"><RefreshCw aria-hidden="true" size={15} className="animate-spin" />Updating transactions; your current data remains visible.</p>}
...
function TransactionDetail({ transactionId, onClose }: { transactionId: string; onClose: () => void }) {
  ...
  return <section aria-labelledby="transaction-detail-title" className="rounded-2xl border border-border bg-surface p-6"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-medium text-accent">Transaction details</p><h2 id="transaction-detail-title" className="mt-1 text-xl font-semibold">Transaction details</h2></div><Button variant="ghost" onClick={onClose}>Close</Button></div>{query.isPending && <p role="status" className="mt-5 text-sm text-muted">Loading the latest transaction status…</p>}{query.isError && <div role="alert" className="mt-5 rounded-xl border border-red-200 bg-red-50 p-4 text-sm text-[var(--aries-danger)]">We could not load the latest transaction status. We will not guess what happened. <Button variant="secondary" className="mt-3" onClick={() => void query.refetch()}>Try again</Button></div>}{query.data && <><div className="mt-5 rounded-xl bg-surface-muted p-4"><TransactionRouteSummary transaction={query.data} compact /></div><dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2"><div><dt className="text-muted">Transaction reference</dt><dd className="mt-1 flex flex-wrap items-center gap-2"><span className="break-all font-mono text-xs">{query.data.id}</span><CopyReferenceButton value={query.data.id} /></dd></div><div><dt className="text-muted">Status</dt><dd className="mt-1"><TransactionStatusBadge transaction={query.data} /></dd></div><div><dt className="text-muted">Amount</dt><dd className="mt-1 font-mono font-semibold">{formatMoney(query.data.amount, query.data.currency)}</dd></div><div><dt className="text-muted">Currency</dt><dd className="mt-1">{query.data.currency}</dd></div><div><dt className="text-muted">Source account</dt><dd className="mt-1"><TransactionPartyDisplay party={query.data.fromParty} /></dd></div><div><dt className="text-muted">Destination account</dt><dd className="mt-1"><TransactionPartyDisplay party={query.data.toParty} /></dd></div><div><dt className="text-muted">Created</dt><dd className="mt-1"><time dateTime={query.data.createdAt}>{formatDate(query.data.createdAt)}</time></dd></div>{query.data.completedAt && <div><dt className="text-muted">Completed</dt><dd className="mt-1"><time dateTime={query.data.completedAt}>{formatDate(query.data.completedAt)}</time></dd></div>}</dl><AuditHistoryUnavailable /></>}</section>;
}
function CopyReferenceButton({ value }: { value: string }) {
  ...
  return <Button type="button" variant="ghost" className="min-h-7 px-1.5" aria-label={copied ? "Transaction reference copied" : "Copy transaction reference"} onClick={() => void copy()}>{copied ? <><Check aria-hidden="true" size={14} />Copied</> : <><Copy aria-hidden="true" size={14} />Copy</>}</Button>;
}
function ErrorState({ error, onRetry }: { error: Error; onRetry: () => void }) { const apiError = error instanceof ApiError ? error : null; const denied = apiError?.kind === "forbidden" || apiError?.kind === "unauthorized"; return <div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-6"><div className="flex gap-3"><AlertTriangle aria-hidden="true" className="mt-0.5 text-[var(--aries-danger)]" size={18} /><div><p className="font-medium text-[var(--aries-danger)]">{denied ? "You cannot access these transactions" : "We could not load your transactions"}</p><p className="mt-1 text-sm text-muted">{denied ? "Check your account access or sign in again." : "We cannot confirm the latest transaction data. Try again when the service is available."}</p><Button variant="secondary" className="mt-4" onClick={onRetry}>Try again</Button></div></div></div>; }
function ReversalDialog({ transaction, result, isPending, error, returnFocusRef, onClose, onConfirm }: { transaction: Transaction; result?: Transaction; isPending: boolean; error: Error | null; returnFocusRef: RefObject<HTMLButtonElement | null>; onClose: () => void; onConfirm: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/30 p-6" role="presentation"><section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="reversal-title" aria-describedby="reversal-description" onKeyDown={handleKeyDown} className="w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-xl"><div className="flex items-start gap-3"><CheckCircle2 aria-hidden="true" className="mt-0.5 text-[var(--aries-warning)]" size={19} /><div><h2 id="reversal-title" className="text-lg font-semibold">Review reversal</h2><p id="reversal-description" className="mt-2 text-sm leading-6 text-muted">This will request a reversal for the completed transaction below. It will remain completed until the service confirms the reversal.</p><dl className="mt-4 space-y-2 rounded-xl bg-surface-muted p-4 text-sm"><div className="flex justify-between gap-4"><dt className="text-muted">Amount</dt><dd className="font-mono font-semibold">{formatMoney(transaction.amount, transaction.currency)}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted">Transaction</dt><dd className="font-mono text-xs">{transaction.id}</dd></div></dl>{isPending && <p role="status" className="mt-3 text-sm text-[var(--aries-warning)]">Submitting the reversal… The service has not confirmed its outcome yet.</p>}{result && <p role="status" className="mt-3 text-sm text-muted">{completed ? "Reversal confirmed." : lifecycle?.kind === "pending" ? "Reversal accepted and still pending." : `Current status: ${lifecycle?.label ?? "Unavailable"}.`}</p>}{error && <p role="alert" className="mt-3 text-sm text-[var(--aries-danger)]">{financialMutationErrorMessage(error, "We could not confirm the reversal status. Check it before trying again.")}</p>}<div className="mt-6 flex justify-end gap-2"><Button ref={closeRef} variant="ghost" onClick={onClose} disabled={isPending}>{result || error ? "Close" : "Cancel"}</Button>{!result && <Button variant="danger" onClick={onConfirm} disabled={isPending}>{isPending ? <><Clock3 aria-hidden="true" size={16} className="mr-2 animate-pulse" />Submitting…</> : "Request reversal"}</Button>}</div></div></div></section></div>;
}
...
function RefundDialog({ transaction, result, isPending, error, returnFocusRef, onClose, onConfirm }: { transaction: Transaction; result?: Transaction; isPending: boolean; error: Error | null; returnFocusRef: RefObject<HTMLButtonElement | null>; onClose: () => void; onConfirm: (amount: string, description?: string) => void }) {
  ...
  const lifecycle = result ? toTransactionLifecycle(result) : null;
  return <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/30 p-6" role="presentation"><section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="refund-title" aria-describedby="refund-description" onKeyDown={handleKeyDown} className="w-full max-w-md rounded-2xl border border-border bg-surface p-6 shadow-xl"><h2 id="refund-title" className="text-lg font-semibold">Review refund</h2><p id="refund-description" className="mt-2 text-sm leading-6 text-muted">This will request a refund. The original transaction stays unchanged until the service confirms the refund.</p><dl className="mt-4 space-y-2 rounded-xl bg-surface-muted p-4 text-sm"><div className="flex justify-between gap-4"><dt className="text-muted">Original amount</dt><dd className="font-mono font-semibold">{formatMoney(transaction.amount, transaction.currency)}</dd></div><div className="flex justify-between gap-4"><dt className="text-muted">Refund currency</dt><dd>{transaction.currency}</dd></div></dl><label htmlFor="refund-amount" className="mt-4 block text-sm font-medium">Refund amount</label><input id="refund-amount" inputMode="decimal" value={amount} onChange={event => setAmount(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 font-mono" aria-describedby="refund-help" disabled={isPending || Boolean(result)} /><p id="refund-help" className="mt-1 text-xs text-muted">Enter the exact amount to refund. The service will check the remaining refundable amount.</p><label htmlFor="refund-description" className="mt-4 block text-sm font-medium">Reason (optional)</label><input id="refund-description" value={description} onChange={event => setDescription(event.target.value)} className="mt-2 h-10 w-full rounded-lg border border-border bg-surface px-3 text-sm" maxLength={255} disabled={isPending || Boolean(result)} />{isPending && <p role="status" className="mt-3 text-sm text-[var(--aries-warning)]">Submitting the refund… The service has not confirmed its outcome yet.</p>}{result && <p role="status" className="mt-3 text-sm text-muted">Current status: {lifecycle?.label ?? "Unavailable"}. Completed styling is shown only after confirmation.</p>}{error && <p role="alert" className="mt-3 text-sm text-[var(--aries-danger)]">{financialMutationErrorMessage(error, "We could not confirm the refund status. Check it before trying again.")}</p>}<div className="mt-6 flex justify-end gap-2"><Button ref={closeRef} variant="ghost" onClick={onClose} disabled={isPending}>{result || error ? "Close" : "Cancel"}</Button>{!result && <Button variant="danger" onClick={() => onConfirm(amount, description || undefined)} disabled={isPending || !valid}>{isPending ? "Submitting…" : "Request refund"}</Button>}</div></section></div>;
}
