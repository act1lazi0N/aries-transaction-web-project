...
export function TransactionWorkspace({ accountId, transactionId, page, size, sort }: Props) {
  ...
  if (!accountId) return <div className="space-y-5"><section className="max-w-md rounded-2xl border border-border bg-surface p-5"><AccountSelect accounts={accountsQuery.data ?? []} value={accountId} onChange={nextAccountId => updateUrl({ accountId: nextAccountId, page: 0, transactionId: undefined })} label="Account" id="transactions-account" isLoading={accountsQuery.isPending} isFetching={accountsQuery.isFetching} error={accountsQuery.error} onRetry={() => void accountsQuery.refetch()} /></section><EmptyState title="Choose an account to view transactions" detail="We only show transactions for an account available to you." /></div>;
