...
import { financialMutationErrorMessage } from "@/lib/api/errors";
...
export function TransferWorkflow({ initialAccountId }: { initialAccountId?: string }) {
  ...
  return <AuthGate><div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.72fr)]">
    <form onSubmit={submitDraft} className="space-y-5 rounded-2xl border border-border bg-surface p-6" aria-describedby="transfer-help">
      <div><h2 className="text-lg font-semibold">Transfer details</h2><p id="transfer-help" className="mt-2 text-sm leading-6 text-muted">Choose your accounts, review the details, then send the transfer once. Your balance changes only after the service confirms it.</p></div>
      ...
      {accountsQuery.data && activeAccounts.length < 2 && <p role="status" className="rounded-lg bg-amber-50 px-3 py-2 text-sm text-[var(--aries-warning)]">You need at least two active accounts to make a transfer.</p>}
      <Button type="submit" disabled={mutation.isPending || activeAccounts.length < 2}>Review transfer details <Send aria-hidden="true" size={16} /></Button>
    </form>
    <section aria-labelledby="transfer-status-title" className="rounded-2xl border border-border bg-surface p-6">
      <div className="flex items-start gap-3"><ShieldAlert aria-hidden="true" className="mt-0.5 text-[var(--aries-warning)]" size={19} /><div><h2 id="transfer-status-title" className="font-semibold">Before you send</h2><p className="mt-2 text-sm leading-6 text-muted">A transfer stays pending until the service confirms its final status.</p></div></div>
      {!preview && <p className="mt-6 text-sm text-muted">Your review will appear here before anything is sent.</p>}
      {preview && !mutation.data && <div className="mt-6 space-y-4"><p className="text-sm font-medium">Review before sending</p><dl className="space-y-3 rounded-xl bg-surface-muted p-4 text-sm"><Item label="From" value={accountLabel(preview.fromAccountId)} /><Item label="To" value={accountLabel(preview.toAccountId)} /><Item label="Amount" value={`${preview.amount} ${preview.currency}`} /><Item label="Description" value={preview.description || "None"} /></dl>{validation && <p role="alert" className="text-sm text-[var(--aries-danger)]">{validation}</p>}{mutation.error && <p role="alert" className="text-sm text-[var(--aries-danger)]">{financialMutationErrorMessage(mutation.error, "We could not confirm the transfer status. Check its status before trying again.")}</p>}<div className="flex flex-wrap gap-2"><Button type="button" onClick={confirm} disabled={Boolean(validation) || mutation.isPending}>{mutation.isPending ? <><Clock3 aria-hidden="true" size={16} className="mr-2 animate-pulse" />Submitting…</> : "Send transfer"}</Button><Button type="button" variant="secondary" onClick={() => setPreview(null)} disabled={mutation.isPending}>Edit details</Button></div></div>}
      {mutation.isPending && <p role="status" className="mt-5 text-sm text-[var(--aries-warning)]">Submitting the transfer… The service has not confirmed its outcome yet.</p>}
      {mutation.data && <div className="mt-6 space-y-4"><p role="status" className="text-sm font-medium">{lifecycle?.kind === "succeeded" ? "Transfer confirmed." : lifecycle?.kind === "pending" ? "Transfer accepted and is still pending." : `Current status: ${lifecycle?.label ?? "Unavailable"}.`}</p><dl className="space-y-3 rounded-xl bg-surface-muted p-4 text-sm"><Item label="Transaction" value={mutation.data.id} /><Item label="Status" value={lifecycle?.label ?? "Unavailable"} /><Item label="Amount" value={`${mutation.data.amount} ${mutation.data.currency}`} /></dl><Button type="button" variant="secondary" onClick={reset}>Start another transfer</Button></div>}
    </section>
  </div></AuthGate>;
}
