...
export default async function ControlsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <AppShell><AuthGate><section className="space-y-8"><div><p className="text-sm font-medium text-accent">Controls</p><h1 className="mt-1 text-3xl font-semibold tracking-tight">Operational controls</h1><p className="mt-3 max-w-2xl text-muted">Run backend-authoritative reconciliation checks and inspect exceptions without inventing ledger state.</p></div><ReconciliationWorkspace key={params.runId ?? "new"} initialRunId={params.runId} /></section></AuthGate></AppShell>;
}
